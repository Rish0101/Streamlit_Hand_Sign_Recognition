...
# Streamlit UI
def main():
    st.title("Real-time Gesture Recognition with Streamlit")

    # Create an empty container to display the image
    image_container = st.empty()

    # Create a sidebar for the FSL Hand Signs image
    st.sidebar.title("FSL Hand Signs")
    fsl_image = cv2.imread("Pictures/Sheet/picture.png")
    st.sidebar.image(fsl_image, channels="BGR", caption="FSL Hand Signs", use_column_width=True)

    # Run the app when the button is clicked
    if st.button("Run Gesture Recognition"):
        global cumulative_accuracy_one_handed, cumulative_accuracy_two_handed, total_frames
        ...
        while True:
                ...
                if hands:
                    if len(hands) == 1:
                        ...
                        try:
                            ...
                            accuracy = (ground_truth_index == index)
                            cumulative_accuracy_one_handed += accuracy

                            # Display accuracy
                            cv2.putText(imgOutput, f"Accuracy (One-handed): {cumulative_accuracy_one_handed / total_frames * 100:.2f}%", (10, 30),
                        ...
                        if distance < join_distance_threshold:
                            ...
                            try:
                                ...
                                # Display accuracy
                                cv2.putText(imgOutput, f"Accuracy (Two-handed): {cumulative_accuracy_two_handed / total_frames * 100:.2f}%", (10, 30),
                                            cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 0), 2)
                            ...
                            try:
                                ...
                                accuracy1 = (ground_truth_index1 == index1)
                                cumulative_accuracy_one_handed += accuracy1

                                # Display accuracy for hand1
                                cv2.putText(imgOutput, f"Accuracy (One-handed): {cumulative_accuracy_one_handed / total_frames * 100:.2f}%", (10, 30),
                                            cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 0), 2)
                            ...
                            try:
                                ...
                                accuracy2 = (ground_truth_index2 == index2)
                                cumulative_accuracy_one_handed += accuracy2

                                # Display accuracy for hand2
                                cv2.putText(imgOutput, f"Accuracy (One-handed): {cumulative_accuracy_one_handed / total_frames * 100:.2f}%", (10, 130),
                                            cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 0), 2)
            ...
            # Update the image container in the Streamlit app
            image_container.image(imgOutput, channels="BGR", use_column_width=True)

            # Exit the program when 'q' is pressed
            if cv2.waitKey(1) == ord('q'):
                break

    # Add a button to pop out an image
